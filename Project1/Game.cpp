...
bool decodeDirection(char ch, int& dir);    // used once in Game
bool recommendMove(const Arena& a, int r, int c, int& bestDir); // used once in Game
int computeDanger(const Arena& a, int r, int c); // used once in recommendMove
...
string Game::takePlayerTurn()
{
    for (;;)
    {
        ...
        else if (playerMove.size() == 1)
        {
            ...
            // recongize when an h is passed in to display the history grid
            else if (tolower(playerMove[0]) == 'h')
            {
                // tell arena's history object to display history grid
                m_arena->history().display();
                // prompt for user response
                cout << "Press enter to continue.";
                // get enter from user
                cin.ignore(10000, '\n');
                // display arena grid again after showing history grid
                m_arena->display("");
            }
        ...
        // Changed this to a conditional
        // Since we dont return anything when we display the history grid,
        // If we reach this point, we need to verify the user puts in a valid move
        if (playerMove != "h")
